...
const Chat = () => {
    ...
    const handleClick = async () => {
        try {
            ...
            setResponse('');
            setError('');
            const Response = await axios.get('http://localhost:8080/ask-ai-options'
                , { params: { prompt } }
            );
    ...
    return (
        <div className='flex gap-5 flex-col items-center justify-center flex-1 w-full px-4'>
            ...
            {!loader && response ?
                <div className="m-4 p-2 border-2 rounded-lg bg-cyan-400 border-cyan-800 max-h-[400px] overflow-y-auto">
                    <h1 className='text-2xl text-fuchsia-900 whitespace-pre-wrap text-left leading-relaxed'>{response}</h1>
                </div> :
                <div></div>
            }
